...
class Solver():
	...
	def report(self, test_input:Path, puzzle_input:Path=None, part_2_test_input:Path=None):
		...
		print(f'\nPart 1:')
		print(f'	Test 1: {self.solve_a(self.read_puzzle_input(test_input))}')
		# If we have a puzzle input, run it through the solver as well.
		if puzzle_input:
			print(f'	Part 1: {self.solve_a(self.read_puzzle_input(puzzle_input))}')
		...
		try:
			...
			test2solution = self.solve_b(self.read_puzzle_input(test_input))
			...
			if puzzle_input:
				print(f'	Part 2: {self.solve_b(self.read_puzzle_input(puzzle_input))}')
